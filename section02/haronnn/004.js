...
function solution(t, s) {
  let answer = 0; // 더한 값을 저장
  let scoring = []; // 1부터 시작, 채점 배열 만듬

  let count = 0; // 1,2,3 만들어줄 요소

  for (let i = 0; i < t; i++) {
    // 문제의 개수 만큼 반복
    // s 배열의 요소가 1과 같을 땐 계속 1씩 증가해준다.
    if (s[i] === 1) {
      count++;
    } else {
      // 그게 아니라면 초기화를 위해서 count를 다시 0으로 만들어준다.
      count = 0;
    }
    scoring.push(count); // push를 반복.
  }
  answer = scoring.reduce((a, b) => a + b, 0); // scoring 배열의 합
  return answer;
}
...
console.log(solution(test, scores));

// for문 안을 작성하는 것이 조금 헷갈렸다. 어떤식으로 증가를 시키고 초기화를 하는지가 난관이었음..
