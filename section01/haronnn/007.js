...
function solution(day, cars) {
  let answer = 0;
  // cars 배열 안의 요소들을 10으로 나눴을 때 나머지가 day와 같다면 answer에 +1
  for (let i of cars) {
    if (i % 10 == day) {
      answer += 1;
    }
...
// for...of
// 반복문(loop) 중 하나로, 배열(array), 문자열(string), Map, Set, NodeList 등과 같은 iterable 객체(iterable object)를 순회할 때 사용

// for...in
// for...in 문은 반복문(loop) 중 하나로, 객체(object)의 모든 열거 가능한 속성(enumerable properties)을 순회할 때 사용
// let obj = { a: 1, b: 2, c: 3 };
