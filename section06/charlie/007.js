...
function solution(arr) {
	const necessary = arr[0].split('');  // 배열로 만들어 shift 활용
	const schedule = arr[1].split('');  // 배열로 만들어 shifth 활용

	while (schedule.length) {  // 모든 수업 다 들을때까지
		if (!necessary.length)  // 필수 과목 다 들었으면 끝
			return "YES";

		if (schedule[0] === necessary[0]) {  // 지금 듣는 과목이 필수 과목이면 필수과목 shift
			necessary.shift();
		}
		schedule.shift();  // 수업 하나 들을때마다 shift
	}

	if (necessary.length)  // 필수과목 남은게 있나요
		return "NO";
	return "YES";
}
...
const testB = ["ABCDEFGHIJ", "ZXYABCDEFGHIJ"];
console.log(solution(testB)); // YES

const testC = ["ABCDE", "EABCDNMKL"];
console.log(solution(testC)); // NO

const testD = ["ABCDE", "ACDEB"];
