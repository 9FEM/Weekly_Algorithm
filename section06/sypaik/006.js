...
function solution(n, k) {
	let princes = []; // 왕자들을 저장할 배열
	for (let i = 1; i <= n; i++) {
	  princes.push(i); // 1부터 n까지의 왕자들을 배열에 추가
	}
  
	let idx = 0;
	while (princes.length > 1) { // 배열의 길이가 1이 될 때까지 반복
	  idx = (idx + k - 1) % princes.length; // k번째 왕자를 찾기 위한 인덱스 계산
	  princes.splice(idx, 1); // k번째 왕자를 배열에서 제거
	}
  
	return princes[0]; // 마지막까지 남은 왕자가 공주를 구하러 갈 수 있다
  }
