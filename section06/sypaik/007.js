...
function solution(course, plan) {
	const courseArr = course.split('');
	for(let x of plan) {
		// 필수 과목이 설계에 포함되어있다면
		if(courseArr.includes(x)) {
			// 앞에서 뺄 때 순서가 다르면
		if(courseArr.shift() !== x) return "NO";
	}
}
	if(courseArr.length > 0) return "NO"; // 다 빼내지 않고 남아있는 경우
	return "YES";
}

const testA = ["CBA", "CBDAGE"];
console.log(solution(testA[0], testA[1])); // YES

const testB = ["ABC", "ACBD"];
console.log(solution(testB[0], testB[1])); // NO

const testC = ["CIDER", "CDIER"];
console.log(solution(testC[0], testC[1])); // NO