...
function solution(N, numbers) {
  let arr = numbers.map((num) => num.toString().split('')); // 자연수를 각각 문자열로 변환
  let max = 0; // 최대 자릿수의 합
  let answer = 0; // 자릿수의 합이 최대인 자연수

  for (let i = 0; i < arr.length; i++) {
    const digitSum = arr[i].reduce((acc, cur) => acc + parseInt(cur), 0); // 자릿수 누적합 + 숫자로 변환하기
    if (max < digitSum) {
      max = digitSum;
      answer = numbers[i];
...
const B = 6;
const testB = [902, 92, 236, 155, 344, 650];
